function void = plot_virulence_fitness_tradeoff_subset_innate(s)
...
for i = 1:k
    ...
    [disease_SI(i,:), ~, TP_SI(i,:), ~, ~] = calculate_PI_fitness_disease(phenotype, s, params, .01);
    i
end
...
[disease_SI_mean, ~, TP_SI_mean, ~, ~] = calculate_PI_fitness_disease(phenotype,s, params, .01);
...
% save('TP_PI_D1_alt_SI', 'TP_PI')
% save('TP_SI_D_alt_SI1', 'TP_SI')
% 
% save('disease_PI_D1_alt_SI', 'disease_PI')
% save('disease_SI_D1_alt_SI', 'disease_SI')
...
xlabel('viral production rate (genome copies per cell per day)'); ylabel('peak viral load (log copies/ml)')
set(gca, 'FontSize', 14)
