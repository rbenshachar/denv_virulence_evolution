function void = plot_model_dynamics_innate(void)

% FIG. S3
% model dynamics of innate model 
...
load('param_estimates_low_innate');

params.q_PI =  param_estimates(1);  
params.q_SI = param_estimates(2); 
...
params.beta = params.beta_PI - 2*c*params.beta_PI;
...
[T_SI, Y_SI] = ode45(@(t,y)PI(t, y, params),params.time_start:.01:params.time_end, [params.Xinit  params.Yinit params.Vinit params.Ninit]);  
V = log10(Y_SI(:,3));
...
params.beta = params.beta_SI - 2*c*params.beta_SI; 
[~, Y2_SI] = ode45(@(t,y)PI(t, y, params),params.time_start:.01:params.time_end, [params.Xinit  params.Yinit params.Vinit params.Ninit]);  

params.beta = params.beta_SI + 2*c*params.beta_SI; 
[~, Y3_SI] = ode45(@(t,y)PI(t, y, params),params.time_start:.01:params.time_end, [params.Xinit  params.Yinit params.Vinit params.Ninit]);  
...
ylabel('viral load (log genome copies per ml)'); title('1^° infection'); ylim([2.5,11]);
...
title('2^° infection');  ylim([2.5, 11]); xlim([0, 15]);
xlabel('time since infection (days)') 
set(gca, 'FontSize', 16)
